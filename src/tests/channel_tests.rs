...
async fn test_total_messages() {
    ...
    let (tx, mut rx) = mpsc_channel_with_total(2, &metrics.queue_size, metrics.total_messages.as_ref().unwrap());
...
async fn test_send_backpressure() {
    ...
    let registry = Registry::new();
    let metrics = ChannelMetrics::new_basic("test_backpressure", "test backpressure", &registry).unwrap();
...
async fn test_send_backpressure_multi_senders() {
    ...
    let registry = Registry::new();
    let metrics = ChannelMetrics::new_basic("test_multi", "test multiple senders", &registry).unwrap();
...
async fn test_reserve_backpressure() {
    ...
    let registry = Registry::new();
    let metrics = ChannelMetrics::new_basic("test_reserve", "test reserve backpressure", &registry).unwrap();
...
async fn test_reserve_and_drop() {
    let registry = Registry::new();
    let metrics = ChannelMetrics::new_basic("test_reserve_drop", "test reserve and drop", &registry).unwrap();
...
async fn test_with_permit_cancel_safety() {
    let registry = Registry::new();
    let metrics = ChannelMetrics::new_basic("test_cancel", "test cancel safety", &registry).unwrap();
